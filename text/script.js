...
function upload(file) {
  ...
  if (group !== "") {
    let path = "azure/text/" + group + "/upload";
    ...
    function uploadFinished() {
      ...
      db.collection("azure_text_analytics_requests").add({
        group: group,
        file: file.name,
        time: Date.now(),
        status: 'open',
        ...
        .then(function (docRef) {
          console.log("Text Analytics Entry created: ", docRef.id);
        })
...
async function getTweetsForGroup(group) {

  return db.collection("azure_text_analytics_requests").where("group", "==", group).get()
    .then(snapshot => {
    ...
    .catch(err => {
      console.log('Error getting documents to sum transactions', err);
    });
}
...
async function loginChanged(user) {
  ...
  if (!user) {
    ...
    numTransactions = await getTweetsForGroup(group);
    document.querySelector("#numTransactions").innerHTML = "Ihr habt bereits <b>" + numTransactions + "</b> von 250 Tweets analysieren lassen.";
